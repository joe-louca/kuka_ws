...
class CopControl:
    ...
    def move_cmd(self):
        if len(self.commandsList) > 0:                  # If there are commands to act on
            ...
            j6 =  35*pi/180
            j7 =  0*pi/180
            self.kuka_pos = [505, 455, -896, 1.249, -1.007, 2.868] #self.iiwa.sendEEfPositionGetActualEEFpos(pos_cmd)         # Send command  & get pos
            self.kuka_jpos = [j1,j2,j3,j4,j5,j6,j7] #self.iiwa.getJointsPos()   # Get joint positions
