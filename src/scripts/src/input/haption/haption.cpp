...
class listener
  {
	...
	listener::ft_delay[1] = data->twist.linear.x;
	listener::ft_delay[2] = data->twist.linear.y;
	listener::ft_delay[3] = data->twist.linear.z;

	listener::ft_delay[4] = data->twist.angular.x;
	listener::ft_delay[5] = data->twist.angular.y;
	listener::ft_delay[6] = data->twist.angular.z;
    }
    ...
    void Publish_V(float* cmd, ros::Publisher ros_pub, float clutch_counter)
    ...
    if(func_result)
    {
	    ...
	    {        
			...
			{	
				...
				for(int i=0;i<3;i++) 
				{
					ft_set[i] = list.ft_delay[i+1]*ft_factor*(ft_user_scale/100.0);
					ft_set[i+3] = list.ft_delay[i+4]*ft_factor*(ft_user_scale/100.0);
				}
